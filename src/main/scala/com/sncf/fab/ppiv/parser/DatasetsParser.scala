  ...
  def parseTgaTgdDataset(row: Row): TgaTgdInput = {
    try {
      ...
      case e => {
        ...
        PpivRejectionHandler.handleRejectionError("KO","",TraitementPPIVDriver.startTimePipeline.toString(),"", "Parsing des fichiers Input. Exception: " + e)
        null
      }
    }
  }
  ...
  def parseRefGares(row: Row): ReferentielGare = {
    try {
      ...
      case e => {
        val sw = new StringWriter
        e.printStackTrace(new PrintWriter(sw))
        PpivRejectionHandler.handleRejectionError("KO","",TraitementPPIVDriver.startTimePipeline.toString(),"", "Parsing du fichier Référentiel. Exception: " + e)
        null
      }
    }
  }
