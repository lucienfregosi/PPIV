...
object AppConf extends Serializable{
  ...
  val REJECTED_FIELD = conf.getString("refinery") + conf.getString("reject_field") + (Conversion.nowToDateTime().getMillis) + ".csv"
  val REJECTED_CYCLE = conf.getString("refinery") + conf.getString("reject_cycle") + (Conversion.nowToDateTime().getMillis) + ".csv"
