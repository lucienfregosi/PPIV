...
trait SourcePipeline extends Serializable {
  ...
  def start(sc : SparkContext, sqlContext : SQLContext, timeToProcess: DateTime): DataFrame = {
    ...
    val theDateoOF16Aout =   Conversion.getDateTime(2017, 8, 16, 23, 0, 0)
    val theDayOf16Aout = BuildCycleOver.loadDataFullPeriod(sc, sqlContext, "TGA", theDateoOF16Aout)
    ...
    hiveContext.sql("Create TABLE ppiv_ref.iv_tgatgdInput_4 as select * temp")
